...
export function getMelodicArrangementEntries(track: MelodicTrack): MelodicArrangementEntry[] {
  return track.steps.flatMap((step, index) => {
    ...
    const pulseStep = track.kind === "pulse" ? track.steps[index] ?? null : null;
...
export function resizeMelodicTrackStep(
  ...
  const previousEntry = entryIndex > 0 ? entries[entryIndex - 1] ?? null : null;
  const nextEntry = entryIndex < entries.length - 1 ? entries[entryIndex + 1] ?? null : null;
...
function normalizeArrangementNote(rawNote: string): NoteValue | null {
  ...
  const [rawNoteName = "", rawAccidental = "", rawOctave = ""] = match.slice(1);
  const normalizedNote = `${rawNoteName.toUpperCase()}${rawAccidental}${rawOctave}` as NoteValue;
...
function parseNoiseTriggerDescriptor(
  ...
  return {
    ok: true,
    mode: ((match[1] ?? "").toLowerCase() || "long") as NoiseTrack["steps"][number]["mode"],
    periodIndex,
  };
}
